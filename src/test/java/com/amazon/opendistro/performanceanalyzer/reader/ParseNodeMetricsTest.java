...
public class ParseNodeMetricsTest extends AbstractReaderTest {
    public ParseNodeMetricsTest() throws SQLException, ClassNotFoundException {
        super();
    }
    ...
    public void testParseMasterPendingMetrics() throws Exception {
        ...
        for (Map.Entry<MetricName, NavigableMap<Long, MemoryDBSnapshot>> entry : mp
            ...
            Field<Double> field = DSL.field(
                    MasterPendingValue.pendingTasksCount.toString(),
                    Double.class);
    ...
    private void verifyDiskMetrics(
        ...
        for (Map.Entry<MetricName, NavigableMap<Long, MemoryDBSnapshot>> entry : nodeMetricsMap
            ...
            fields[0] = DSL.field(DiskValue.util.toString(), Double.class);
            fields[1] = DSL.field(DiskValue.wait.toString(), Double.class);
            fields[2] = DSL.field(DiskValue.srate.toString(), Double.class);

            Result<Record> resRecordDiskXvda = curSnap.fetchMetric(
                    getDimensionEqCondition(DiskDimension.name, diskXvda),
                    fields);
            ...
            Result<Record> resRecordNvme0n1 = curSnap.fetchMetric(
                    getDimensionEqCondition(DiskDimension.name, diskNvme0n1),
                    fields);
