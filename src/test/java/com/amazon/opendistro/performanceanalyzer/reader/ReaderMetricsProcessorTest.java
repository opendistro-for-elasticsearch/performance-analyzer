...
public class ReaderMetricsProcessorTest extends AbstractReaderTest {
    ...
    public void testReaderMetricsProcessor() throws Exception {
        ...
        Result<Record> res = mp.getMetricsDB().getValue().queryMetric(Arrays.asList("cpu"),
                Arrays.asList("sum"),
                Arrays.asList("shard", "index", "operation"));
        Double shardFetchCpu = 0d;
        for (Record record: res) {
            if (record.get("operation").equals("shardfetch")) {
                shardFetchCpu = Double.parseDouble(record.get("cpu").toString());
                break;
            }
        }
    ...
    public void testAlignNodeMetrics() throws Exception {
        ...
        Field<Double> valueField = DSL.field(
                MasterPendingValue.pendingTasksCount.toString(), Double.class);
    ...
    public void testEmitNodeMetrics() throws Exception {
        ...
        Result<Record> res = db
                .queryMetric(MasterPendingValue.pendingTasksCount.toString());
